...
app.all(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // allow client to provide email_reg, has_reg and ServerName (for register flow)
      const emailReg = typeof formData.email_reg === 'string' && formData.email_reg.trim() !== ''
        ? formData.email_reg
        : 'undefined';
      const hasReg = typeof formData.has_reg === 'string' && formData.has_reg.trim() !== ''
        ? formData.has_reg
        : '0';
      const serverNameFromForm = typeof formData.ServerName === 'string' && formData.ServerName.trim() !== ''
        ? formData.ServerName
        : undefined;

      const serverName = serverNameFromForm || req.hostname || (req.headers.host as string) || 'unknown';

      const token = Buffer.from(
        `_token=${_token}&growId=${growId}&password=${password}&email_reg=${emailReg}&has_reg=${hasReg}&ServerName=${serverName}`,
      ).toString('base64');
...
app.all(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // parse query-string style token into map
      const pairs = decodeRefreshToken.split('&').map((p) => p.split('='));
      const map = new Map<string, string>(pairs.map(([k, v]) => [k, v]));

      // replace _token with base64(clientData)
      map.set('_token', Buffer.from(clientData).toString('base64'));

      // decide if this is a registration (clientData contains an email)
      const isEmail = typeof clientData === 'string' && clientData.includes('@');
      map.set('email_reg', isEmail ? clientData : 'undefined');
      map.set('has_reg', isEmail ? '1' : '0');

      // append/overwrite ServerName at the end
      const serverName = req.hostname || (req.headers.host as string) || 'unknown';
      map.set('ServerName', serverName);

      const rebuilt = Array.from(map.entries()).map(([k, v]) => `${k}=${v}`).join('&');
      const token = Buffer.from(rebuilt).toString('base64');
